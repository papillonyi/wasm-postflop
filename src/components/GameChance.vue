<template>
  <div class="flex w-full h-full">
    <div
      ...
      style="flex: 4"
    >
      <button class="ml-3 button-base button-blue" @click="randomDeal">
        Random-card
      </button>
      <div
        v-for="suit in 4"
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const randomDeal = () => {
      console.log(
        gameStore.playersInfo[0].card1,
        gameStore.playersInfo[0].card2,
        props.selectedChance.cards
      );
      const cardsList = props.selectedChance.cards.filter((item) => {
        return (
          !item.isDead &&
          item.card !== gameStore.playersInfo[0].card1 &&
          item.card !== gameStore.playersInfo[0].card2 &&
          item.card !== gameStore.playersInfo[1].card1 &&
          item.card !== gameStore.playersInfo[1].card2
        );
      });

      const randomOne = getRandomItem(cardsList);
      console.log(cardsList, randomOne.card);
      deal(randomOne.card);
    };
